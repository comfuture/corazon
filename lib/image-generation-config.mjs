export const normalizeImageGenerationFeatureConfig = (original) => {
  ...
  const featuresAnyDottedPattern = /^(?:"features"|features)\s*\./i
  const featuresImageGenerationDottedPattern = /^(?:"features"|features)\s*\.\s*(?:"image_generation"|'image_generation'|image_generation)\s*=/i
  const featuresImageGenerationDottedTruePattern = /^(?:"features"|features)\s*\.\s*(?:"image_generation"|'image_generation'|image_generation)\s*=\s*true(?:\s+#.*)?$/i
  const featuresInlinePattern = /^(\s*)(?:"features"|features)\s*=\s*\{(.*)\}(\s*(?:#.*)?)$/i
  ...
  for (const line of lines) {
    ...
    if (!inFeatures && featuresImageGenerationDottedPattern.test(trimmed)) {
      sawFeatures = true
      sawDottedFeatures = true
      imageGenerationSet = true
    ...
    if (!inFeatures && featuresAnyDottedPattern.test(trimmed)) {
      sawFeatures = true
      sawDottedFeatures = true
      nextLines.push(line)
      continue
  ...
  if (!sawFeatures) {
    ...
    nextLines.push('image_generation = true')
    changed = true
  } else if (!imageGenerationSet && sawDottedFeatures) {

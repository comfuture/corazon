...
## 목표
- app-server 모드에서 `manage-workflows`, `shared-memory`를 네이티브 dynamic tool로 제공한다.
...
- `server/utils/codex-client/native-tools.ts`
  ...
  - 인자 검증/정규화
  - `shared-memory`, `manage-workflows` 핸들러
...
### Phase 1: 네이티브 툴 모듈
- [ ] `native-tools.ts` 추가
- [ ] `shared-memory` 커맨드(`ensure/search/upsert`) 구현
- [ ] `manage-workflows` 커맨드(`list/create/update/delete/from-text/apply-text`) 구현
- [ ] 동적 툴 입력 스키마 정의

### Phase 2: app-server 하네스 연결
- [ ] `app-server-protocol.ts`에 dynamic tool dispatcher 연결
- [ ] `thread/start`에 `dynamicTools` 전달
- [ ] 미지원 툴/입력 오류 처리 통일

### Phase 3: 메인 루프 정합화
- [ ] app-server 모드 라우팅 힌트 보강 (`chat-turn.ts`)
- [ ] shared-memory 안내 문구 보강 (`agent-bootstrap.ts`)

### Phase 4: 검증/마무리
- [ ] `pnpm lint`
- [ ] `pnpm check` 실행 시도 및 결과 기록
- [ ] `pnpm typecheck`
- [ ] 단계별 atomic 커밋 완료

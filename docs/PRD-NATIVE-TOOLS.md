...
## 목표
- app-server 모드에서 `manageWorkflow`, `sharedMemory`를 네이티브 dynamic tool로 제공한다.
...
- `server/utils/codex-client/native-tools.ts`
  ...
  - 인자 검증/정규화
  - `sharedMemory`, `manageWorkflow` 핸들러
...
- [x] `sharedMemory` 커맨드(`ensure/search/upsert`) 구현
- [x] `manageWorkflow` 커맨드(`list/create/update/delete/from-text/apply-text`) 구현
- [x] 동적 툴 입력 스키마 정의

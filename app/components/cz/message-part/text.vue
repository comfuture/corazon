...
const { rewriteContentWithLocalFilePreviews } = useLocalFilePreview()
...
watch(
  ...
  async ([role, text], _previousValue, onCleanup) => {
    ...
    const rewritten = await rewriteContentWithLocalFilePreviews(sourceText, resolvedThreadId.value)
    if (!cancelled) {
